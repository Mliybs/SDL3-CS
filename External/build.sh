...
if [[ $BUILD_PLATFORM == 'Emscripten' ]]; then
    ...
    SDL_SHARED_FLAG=OFF
    SDL_STATIC_FLAG=ON
    EXTRA_CMAKE_FLAGS='-DCMAKE_STATIC_LIBRARY_PREFIX="lol" -DBUILD_SHARED_LIBS=OFF -DCMAKE_FIND_ROOT_PATH_MODE_PACKAGE=BOTH -DSDL_TESTS=OFF -DSDL_TEST_LIBRARY=OFF -DSDLTTF_SAMPLES=OFF -DSDLIMAGE_SAMPLES=OFF -DSDLMIXER_SAMPLES=OFF'
    EMSCRIPTEN_FLAGS='-DCMAKE_STATIC_LIBRARY_PREFIX=""'
else
    CMAKE_WRAPPER=""
    SDL_SHARED_FLAG=ON
    SDL_STATIC_FLAG=OFF
