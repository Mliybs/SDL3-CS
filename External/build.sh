...
if [[ $BUILD_PLATFORM == 'Emscripten' ]]; then
    ...
    SDL_SHARED_FLAG=OFF
    SDL_STATIC_FLAG=ON
    EXTRA_CMAKE_FLAGS='-DCMAKE_PROJECT_INCLUDE_BEFORE=/home/runner/work/SDL3-CS/SDL3-CS/External/wasm.cmake -DCMAKE_SHARED_LIBRARY_PREFIX= -DCMAKE_STATIC_LIBRARY_PREFIX= -DBUILD_SHARED_LIBS=OFF -DCMAKE_FIND_ROOT_PATH_MODE_PACKAGE=BOTH -DSDL_TESTS=OFF -DSDL_TEST_LIBRARY=OFF -DSDLTTF_SAMPLES=OFF -DSDLIMAGE_SAMPLES=OFF -DSDLMIXER_SAMPLES=OFF'
else
    CMAKE_WRAPPER=""
    SDL_SHARED_FLAG=ON
    SDL_STATIC_FLAG=OFF
